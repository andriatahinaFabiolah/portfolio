...
const projects = [
  {
    ...
    description: "Application de suivi en temps réel du matériel informatique — affectation aux directions, automatisation des processus internes et optimisation de la gestion des équipements de l'institution.",
    accent: "#6366f1",
  ...
  },
  {
    ...
    description: "Supervision des virements bancaires automatisés via triggers MySQL. Interfaces distinctes admin & client avec historique complet des transactions et gestion des flux financiers.",
    accent: "#0ea5e9",
  ...
  },
  {
    ...
    description: "Visualisation interactive de l'algorithme du sac à dos — programmation dynamique O(n·W). Outil pédagogique pour comprendre et explorer les fondamentaux algorithmiques.",
    accent: "#f59e0b",
...
export default function App() {
  ...
  /* ── Carousel helpers ── */
  const goNext = (pi) => setSlideIndices(prev =>
    prev.map((s, i) => i === pi ? (s + 1) % projects[pi].images.length : s)
  );
  const goPrev = (pi) => setSlideIndices(prev =>
    prev.map((s, i) => i === pi ? (s - 1 + projects[pi].images.length) % projects[pi].images.length : s)
  );
  const goTo = (pi, di) => setSlideIndices(prev =>
    prev.map((s, i) => i === pi ? di : s)
  );
    ...
    <div style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", background: c.bg, color: c.text, minHeight: "100vh", transition: "background 0.3s, color 0.3s" }}>
      ...
      <style>{`
        ...
        .mobile-menu {
          ...
          padding: 16px 6vw 24px; flex-direction: column; gap: 4px; z-index: 99;
        }
      ...
      <section id="about" className="hero-grid" style={{
        ...
        <Reveal delay={200}>
          <div className="hero-photo" style={{position:"relative",zIndex:1}}>
            ...
            <div style={{position:"relative",zIndex:1,borderRadius:"20px",overflow:"hidden",aspectRatio:"3/4",background:c.border,display:"flex",alignItems:"center",justifyContent:"center",border:`4px solid ${dark?"#1e293b":"#fff"}`,boxShadow:"0 20px 60px rgba(0,0,0,0.1)"}}>
              ...
              <Reveal key={p.id} delay={pi * 100}>
                <div style={{background:c.bgAlt,border:`1.5px solid ${c.border}`,borderRadius:"16px",overflow:"hidden",transition:"all 0.3s ease",height:"100%",display:"flex",flexDirection:"column"}}
                  ...
                  <div style={{padding:"20px 24px",flex:1,display:"flex",flexDirection:"column"}}>
                    ...
                    {p.badge && (
                      <span style={{display:"inline-block",fontSize:"11px",fontWeight:600,color:"#16a34a",background:"#dcfce7",padding:"3px 10px",borderRadius:"100px",marginBottom:"8px",alignSelf:"flex-start"}}>✓ {p.badge}</span>
                    )}
                    <p style={{fontSize:"13px",color:c.textMuted,margin:"0 0 14px",lineHeight:1.6,flex:1}}>{p.description}</p>
                    <div style={{display:"flex",gap:"6px",flexWrap:"wrap"}}>
                      {p.tech.map(t => (
                        <span key={t} style={{fontSize:"11px",fontWeight:600,color:c.textMuted,background:dark?"#334155":"#f5f5f4",border:`1px solid ${c.border}`,padding:"3px 10px",borderRadius:"6px"}}>{t}</span>
                      ))}
                    </div>
                  </div>
                </div>
          ...
          <div className="skills-grid" style={{display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(240px, 1fr))",gap:"24px"}}>
            {Object.entries(skills).map(([category, items], ci) => (
              <Reveal key={category} delay={ci * 80}>
                <div style={{background:c.bg,border:`1.5px solid ${c.border}`,borderRadius:"16px",padding:"24px"}}>
                  <h3 style={{fontSize:"13px",fontWeight:700,color:c.textHint,textTransform:"uppercase",letterSpacing:"1px",marginBottom:"16px"}}>{category}</h3>
                  <div style={{display:"flex",flexWrap:"wrap",gap:"8px"}}>
                    {items.map(skill => (
                      <span key={skill} style={{fontSize:"13px",fontWeight:600,color:c.text,background:dark?"#334155":"#f5f5f4",border:`1.5px solid ${c.border}`,padding:"6px 14px",borderRadius:"8px",transition:"all 0.2s",cursor:"default"}}
      ...
      <section id="contact" style={{padding:"100px 6vw",background:c.bg}}>
        <div style={{maxWidth:"680px",margin:"0 auto"}}>
          <Reveal>
            ...
            <h2 style={{fontSize:"clamp(28px, 4vw, 44px)",fontWeight:800,letterSpacing:"-1px",marginTop:"8px",marginBottom:"8px",color:c.text}}>Me contacter</h2>
          ...
          <Reveal delay={100}>
            {sent ? (
              ...
              <form ref={formRef} onSubmit={handleSubmit} style={{display:"flex",flexDirection:"column",gap:"16px"}}>
                ...
                {error && (
                  <div style={{background:"#fef2f2",border:"1px solid #fecaca",borderRadius:"8px",padding:"12px 16px",fontSize:"14px",color:"#dc2626"}}>⚠️ {error}</div>
                )}
